...
def get_chatgpt_response(prompt, max_tokens=500):
    response = openai.Completion.create(
        ...
        prompt=prompt,
        temperature=0.4,  # Higher temperature for more randomness
...
def generate_business_guide(user_business):
    # Generate a step-by-step guide for the specified business type
...
while True:
    ...
    if user_response == 'yes':
        ...
        # Use ChatGPT to respond to user's questions about their existing business
        while user_question.lower() != 'exit':
            prompt = f"{user_question}"
            response = get_chatgpt_response(prompt, max_tokens=200)
            display_response_with_delay(response, delay=3)  # Delay of 3 seconds after the response
            user_question = input("You: ")
        ...
        # Generate and display a step-by-step guide for the specified business
        guide = generate_business_guide(user_business)
        display_response_with_delay(guide, delay=5)  # Delay of 5 seconds before displaying the guide

        # Use ChatGPT to respond to user's questions about the guide
        print(f"{chatbot_identity}: Do you have any specific questions about the steps mentioned in the guide?")
        user_question = input("You: ")

        while user_question.lower() != 'exit':
            # Break down the user's question to get more focused responses
            if "step" in user_question.lower():
                step_number = user_question.split()[-1]  # Extract the step number from the question
        ...
        # For any other questions, use ChatGPT to provide responses
